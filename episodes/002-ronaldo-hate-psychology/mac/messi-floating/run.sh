...
render_args=(--background --python-exit-code 1 --python "$packet_dir/messi_floating.py" -- --output "$packet_dir")
case "$mode" in
  build) ;;
...
if "$blender_bin" "${render_args[@]}" > "$log" 2>&1; then
  printf 'Completed: %s/messi-floating.blend\n' "$packet_dir"
  case "$mode" in
    preview) printf 'Five preview images: %s/previews/\n' "$packet_dir" ;;
