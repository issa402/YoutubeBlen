...
def approved_ronaldo(offset):
    ...
    triangles=tessellate_polygon([points]);indices={tuple(v):i for i,v in enumerate(points)}
    mesh=bpy.data.meshes.new('Approved Ronaldo silhouette')
    mesh.from_pydata(points,[],[tuple(v if isinstance(v,int) else indices[tuple(v)] for v in tri) for tri in triangles]);mesh.update()
